...
import { useActivitiesStore, actPaceSec } from '@/entities/activity';
import {
  usePlanStore, isDefaultPlan, buildActivityMap,
  TRAINING_PLAN, TYPE_LABELS, TYPE_COLORS, RACE_DATE, RACE_DIST_KM, RACE_TARGET_MIN, RACE_TARGET_PACE_SEC, HR_ZONES,
  type PlanSession,
} from '@/entities/training-plan';
import { paceSecToStr, fmt, hmFromMin } from '@/shared/lib';
...
function WeekActuals({ weekNum }: { weekNum: number }) {
  ...
  const { plan: storePlan } = usePlanStore();
...
function PlanEditor({ onClose }: { onClose: () => void }) {
  const { plan, setPlan, resetPlan } = usePlanStore();
