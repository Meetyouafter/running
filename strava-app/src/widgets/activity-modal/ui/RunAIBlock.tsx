...
import { RACE_DATE, RACE_DIST_KM, RACE_TARGET_MIN, type PlanSession } from '@/entities/training-plan';
import { fmt, dur, pace, paceSecToStr, escHtml } from '@/shared/lib';
...
export default function RunAIBlock({ detail, streams, ivl, plan }: Props) {
  ...
  async function analyze() {
    ...
    try {
      let text = escHtml(await askGemini(key, prompt));
      ...
      setOut(`<span style="color:#f44336">Ошибка: ${escHtml(String(e))}</span>`);
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className={styles.block}>
      <div className={styles.header}>
        ...
        <input
          ...
          value={key}
          onChange={e => { setKey(e.target.value); saveGeminiKey(e.target.value); }}
        />
